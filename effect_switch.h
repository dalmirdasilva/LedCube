/**
 * LedCube Project
 * 
 * effect_switch.h
 */
 
#ifndef __PIC_LEDCUBE_EFFECT_SWITCH_H
#define __PIC_LEDCUBE_EFFECT_SWITCH_H 1 

#define EFFECT_TEST                  0
#define EFFECT_RAIN                  0
#define EFFECT_PLANBOING             0 // moving planes
#define EFFECT_BLINKY                0 // blinking and fast
#define EFFECT_BOX_SHRING_GROW       0 // corner
#define EFFECT_BOX_WOOPWOOP          0 // center
#define EFFECT_BOINGBOING            0 // making random lines
#define EFFECT_Z_UPDOWN              0 // lines up down
#define EFFECT_AXIS_UPDOWN_RSUSPEND  0 // good
#define EFFECT_BOXSIDE_RAND_SEND     0 // good
#define EFFECT_LOADBAR               0 // load to up
#define EFFECT_SPARKLE               0 // 
#define EFFECT_TELKSTAIRS            0 // diagonal load
#define EFFECT_WORMSQUEEZE           0 // good
#define EFFECT_RANDOM_FILTER         0 //
#define EFFECT_SENDPLANE_RZ          0 
#define EFFECT_SENDVOXELS_RZ         0 // good but I cant see
#define EFFECT_BIT_WALKING           0
#define EFFECT_LINES                 0
#define EFFECT_RIPPLES               0
#define EFFECT_STRING_FLY            1
#define EFFECT_CLOSING_BOX			 1


#endif // __PIC_LEDCUBE_EFFECT_SWITCH_H
